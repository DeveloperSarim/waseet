...
function Eyebrow({ children, style }) {
  return (
    <div style={{ fontSize: 10, fontWeight: 600, color: colors.green, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10, ...style }}>
      {children}
    </div>
  )
}
...
function CheckLine({ children, weight = 2 }) {
  return (
    <span style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 13, color: colors.textSoft }}>
      <svg width={15} height={15} viewBox="0 0 24 24" fill="none" stroke={colors.green} strokeWidth={weight} strokeLinecap="round" strokeLinejoin="round">
...
const heroCards = [
  { w: 230, rot: 'rotate(-3deg)', delay: '0s', shadow: '0 10px 30px rgba(0,0,0,0.07)', h: 120, badge: { t: 'NEW', bg: '#FEF9EC', bd: '#F3E2B8', c: '#92763A' }, comm: '4% Commission', name: 'Olaya Park Towers', loc: 'Al Olaya, Riyadh', price: 'SAR 890K – 5.6M', nameSize: 13 },
  { w: 244, rot: 'scale(1.06)', delay: '.6s', shadow: '0 16px 44px rgba(0,0,0,0.1)', h: 128, badge: { t: 'FEATURED', bg: colors.greenTint, bd: colors.greenTintBorder, c: colors.greenDark }, comm: '3% Commission', name: 'Marsa Heights', loc: 'Al Shati, Jeddah', price: 'SAR 980,000 – 1.6M', nameSize: 14, render: true },
  { w: 230, rot: 'rotate(3deg)', delay: '1.2s', shadow: '0 10px 30px rgba(0,0,0,0.07)', h: 120, badge: { t: 'FEATURED', bg: colors.greenTint, bd: colors.greenTintBorder, c: colors.greenDark }, comm: '2.5% Commission', name: 'Hittin Garden Villas', loc: 'Hittin, Riyadh', price: 'SAR 3.1M – 8.2M', nameSize: 13 },
]
...
function HeroCard({ c }) {
  return (
    <div style={{ transform: c.rot }}>
      <div style={{ animation: 'wfloat 4s ease-in-out infinite', animationDelay: c.delay, width: c.w, background: '#fff', border: `1px solid ${colors.border}`, borderRadius: 14, overflow: 'hidden', boxShadow: c.shadow, textAlign: 'left' }}>
        <div style={{ height: c.h, background: colors.surfaceMuted, backgroundImage: hatch, position: 'relative' }}>
          <span style={{ position: 'absolute', top: 9, left: 9, background: c.badge.bg, border: `1px solid ${c.badge.bd}`, borderRadius: 999, padding: '2px 8px', fontSize: 9, fontWeight: 600, color: c.badge.c, letterSpacing: '0.03em' }}>{c.badge.t}</span>
          <span style={{ position: 'absolute', bottom: 9, right: 9, background: colors.ink, borderRadius: 5, padding: '3px 8px', fontSize: 10, fontWeight: 700, color: '#fff' }}>{c.comm}</span>
          {c.render && <span style={{ position: 'absolute', top: 10, right: 11, fontFamily: 'monospace', fontSize: 8, color: '#B6BAC0' }}>project render</span>}
        </div>
        <div style={{ padding: '11px 13px' }}>
          <div style={{ fontSize: c.nameSize, fontWeight: c.nameSize > 13 ? 700 : 600 }}>{c.name}</div>
...
const marqueeCards = [
  { grad: 'linear-gradient(150deg,#0F3D2E,#1E7A52)', badge: 'FEATURED', comm: '3%', name: 'Palm Residence', loc: 'Al Rawdhah, Jeddah', price: 'SAR 650K – 1.2M' },
  { grad: 'linear-gradient(150deg,#0E4F4A,#2A9D8F)', badge: 'NEW', comm: '4%', name: 'Nesma Villas', loc: 'Al Yasmin, Riyadh', price: 'SAR 2.1M – 3.4M' },
  { grad: 'linear-gradient(150deg,#1F2937,#4B5563)', badge: 'FEATURED', comm: '2.5%', name: 'Aldar Offices Tower', loc: 'Al Olaya, Riyadh', price: 'SAR 1.4M – 4.8M' },
  { grad: 'linear-gradient(150deg,#15803D,#34D058)', badge: 'FEATURED', comm: '3.5%', name: 'Marsa Heights', loc: 'Al Shati, Jeddah', price: 'SAR 980K – 1.6M' },
  { grad: 'linear-gradient(150deg,#3F3A33,#6E6358)', badge: 'NEW', comm: '3%', name: 'Jeddah Gate Townhomes', loc: 'Al Naseem, Jeddah', price: 'SAR 980K – 1.7M' },
  { grad: 'linear-gradient(150deg,#1E293B,#3B5168)', badge: 'FEATURED', comm: '4%', name: 'Sumou Garden Villas', loc: 'Al Shati, Jeddah', price: 'SAR 2.4M – 5.2M' },
]

function MarqueeCard({ c }) {
  const isNew = c.badge === 'NEW'
  return (
    <div style={{ flex: 'none', width: 248, background: '#fff', border: `1px solid ${colors.border}`, borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
      <div style={{ height: 120, background: c.grad, position: 'relative', overflow: 'hidden' }}>
        ...
        <span style={{ position: 'absolute', bottom: 9, right: 9, background: '#fff', borderRadius: 6, padding: '3px 9px', fontSize: 11, fontWeight: 700, color: colors.ink }}>{c.comm} Commission</span>
        <span style={{ position: 'absolute', bottom: 9, left: 11, fontFamily: 'monospace', fontSize: 8, color: 'rgba(255,255,255,0.55)' }}>project render</span>
      </div>
      <div style={{ padding: '11px 13px' }}>
...
const whyCards = [
  { d: 'M11 11m-7 0a7 7 0 1 0 14 0a7 7 0 1 0-14 0M21 21l-4-4', title: 'One private marketplace', body: 'Every approved developer project in one place, with commission shown upfront on every unit type.' },
  { d: 'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10zM9 12l2 2 4-4', title: 'Verified partners only', body: 'Every developer and realtor is reviewed by our admin team before accessing the platform.' },
  { d: 'M12 12m-9 0a9 9 0 1 0 18 0a9 9 0 1 0-18 0M12 7v10M9.5 9.5c0-1 1.1-1.5 2.5-1.5s2.5.5 2.5 1.5-1.1 1.5-2.5 1.5-2.5.5-2.5 1.5 1.1 1.5 2.5 1.5 2.5-.5 2.5-1.5', title: 'Commission always shown upfront', body: 'No negotiating after the deal. Commission rates are locked per unit type before you submit a lead.' },
  { d: 'M3 3v18h18M7 14l3-3 3 3 5-6', title: 'Full tracking dashboard', body: 'Track every lead from submission to close. Know exactly where every deal stands in real time.' },
  { d: 'M12 8m-5 0a5 5 0 1 0 10 0a5 5 0 1 0-10 0M8.5 12.5L7 22l5-3 5 3-1.5-9.5', title: 'Badge & reward system', body: 'Realtors earn Bronze, Silver, Gold, and Platinum badges as they close more deals — unlocking more perks.' },
  { d: 'M13 2L3 14h7l-1 8 10-12h-7l1-8z', title: '24-hour approval', body: 'Apply today. Our team reviews applications within 24 hours — you could be submitting leads tomorrow.' },
]
  ...
  return (
    <div {...hp} style={{ background: '#fff', border: `1px solid ${h ? colors.green : colors.border}`, borderRadius: 12, padding: 22, transition: 'all 200ms ease', transform: h ? 'translateY(-2px)' : 'none' }}>
      <div style={{ width: 42, height: 42, borderRadius: 10, background: colors.ink, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
        <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><path d={c.d} /></svg>
      </div>
...
const propTypes = [
  { d: 'M3 21h18M6 3h7v18M13 9h5v12M9 7v.01M9 11v.01M9 15v.01', name: 'Apartments', count: '18,420 projects' },
  { d: 'M3 11l9-8 9 8M5 10v10h14V10M10 20v-6h4v6', name: 'Villas', count: '9,210 projects' },
  { d: 'M3 21h18M5 21V9l5-4v16M14 21V11l5-3v13', name: 'Townhouses', count: '4,860 projects' },
  { d: 'M2 7h20v14H2zM8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2', name: 'Offices', count: '3,140 projects' },
  { d: 'm8 3 4 8 5-5 5 15H2L8 3z', name: 'Land', count: '2,705 plots' },
]

function TypeCard({ c, navigate }) {
  const [h, hp] = useHover()
  return (
    <div {...hp} onClick={() => navigate('/marketplace')} style={{ background: h ? colors.greenTint : '#fff', border: `1px solid ${h ? colors.green : colors.border}`, borderRadius: 12, padding: 20, cursor: 'pointer', transition: 'all 150ms ease' }}>
      <div style={{ width: 40, height: 40, borderRadius: 10, background: colors.greenTint, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
        <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={colors.green} strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><path d={c.d} /></svg>
      </div>
...
const badges = [
  { icon: '🥉', name: 'Bronze', req: 'New approved realtor', perk: 'Access to all projects', active: true },
...
const trustCards = [
  { d: 'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10zM9 12l2 2 4-4', title: 'Verified partners only', body: 'Every developer and realtor is reviewed and licence-checked by our team before they can list or submit a single lead.' },
  { d: 'M3 11h18v11H3zM7 11V7a5 5 0 0 1 10 0v4', title: 'Commission protected', body: 'Rates are locked per unit before you submit. Waseet holds and verifies every payment, then disburses your share to your bank.' },
  { d: 'M17 14a7 7 0 1 1-5.6-11.3A6 6 0 1 0 17 14zM19 4l.6 1.6L21 6l-1.4.4L19 8l-.6-1.6L17 6z', title: 'Shariah-compliant', body: 'Every deal and commission structure adheres to Islamic Sharia principles — invest and earn without compromising your values.' },
]
...
const steps = [
  { n: '01', title: 'Every developer project, in one private feed.', body: 'Realtors browse exclusive off-plan and ready projects with the commission rate shown upfront on every unit — no portals, no noise.' },
  { n: '02', title: 'Submit a lead, then watch it move to close.', body: 'Send a client lead in seconds and follow it live — New, Viewing, Negotiating, Closed — with every status update in one dashboard.' },
  { n: '03', title: 'Your commission, tracked and paid on time.', body: 'When a deal closes, Waseet verifies the commission and disburses your share straight to your bank — usually within 5–7 business days.' },
]
...
export default function Landing() {
  ...
  const dev = () => navigate('/register/developer')
  const realtor = () => navigate('/register/realtor')

  const statItems = [
    { d: 'M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2M9 7m-4 0a4 4 0 1 0 8 0a4 4 0 1 0-8 0M22 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75', value: '200+', label: 'Verified realtors' },
    { d: 'M3 21h18M6 21V7l6-4 6 4v14M9 11h2M13 11h2M9 15h2M13 15h2', value: '50+', label: 'Developer projects' },
    { d: 'M12 12m-9 0a9 9 0 1 0 18 0a9 9 0 1 0-18 0M8.5 15.5l7-7', value: 'SAR 2M+', label: 'Commission paid' },
    { d: 'M12 21s7-6.5 7-12a7 7 0 1 0-14 0c0 5.5 7 12 7 12zM12 9m-2.5 0a2.5 2.5 0 1 0 5 0a2.5 2.5 0 1 0-5 0', value: '13', label: 'Cities covered', last: true },
  ]
    ...
    <div>
      ...
      <div style={{ background: '#fff', padding: '72px 32px 56px', textAlign: 'center', overflow: 'hidden' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: colors.greenTint, border: `1px solid ${colors.greenTintBorder}`, borderRadius: 999, padding: '5px 14px', marginBottom: 24 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 500, color: colors.greenDark }}>Private · Verified · Commission-first</span>
        </div>
        <h1 className="wa-h1" style={{ fontSize: 48, fontWeight: 700, color: colors.ink, letterSpacing: '-0.04em', lineHeight: 1.08, maxWidth: 780, margin: '0 auto' }}>
          The Private Marketplace<br />Where Developers Meet<br />Their Best <span style={{ color: colors.green }}>Realtors.</span>
        </h1>
        <p style={{ fontSize: 16, color: colors.textSoft, lineHeight: 1.6, maxWidth: 520, margin: '16px auto 8px' }}>
        ...
        <div style={{ fontSize: 13, color: colors.textFaint, marginBottom: 36 }}>Jeddah · Riyadh · Dammam · Mecca · Medina · Khobar</div>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginBottom: 32, flexWrap: 'wrap' }}>
          <CtaButton onClick={dev}>List your project — I'm a Developer</CtaButton>
          <CtaButton variant="outline" onClick={realtor} style={{ borderWidth: 1.5 }}>Join as Realtor — Start Earning</CtaButton>
        </div>
        <div style={{ display: 'flex', gap: 28, justifyContent: 'center', flexWrap: 'wrap' }}>
          <CheckLine>Verified developers only</CheckLine>
          <CheckLine>Licensed realtors</CheckLine>
          <CheckLine>Commission protected</CheckLine>
          <CheckLine>24hr approval</CheckLine>
        </div>
        <div className="wa-hero-cards" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 18, marginTop: 52 }}>
          {heroCards.map((c, i) => <HeroCard key={i} c={c} />)}
        </div>
      </div>
      ...
      <div style={{ background: '#fff', borderTop: `1px solid ${colors.border}`, borderBottom: `1px solid ${colors.border}`, display: 'flex', flexWrap: 'wrap' }}>
        {statItems.map((s, i) => (
          <div key={i} style={{ flex: '1 1 180px', padding: '24px 0', textAlign: 'center', borderRight: s.last ? 'none' : `1px solid ${colors.border}` }}>
            <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={colors.green} strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: 8 }}><path d={s.d} /></svg>
      ...
      <div style={{ background: colors.bg, borderTop: `1px solid ${colors.border}`, padding: '72px 32px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            <Eyebrow>Who is Waseet for?</Eyebrow>
            <h2 style={{ fontSize: 32, fontWeight: 700, letterSpacing: '-0.02em', margin: 0 }}>Built for two sides of every deal.</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 16 }}>
            <ForWhoCard
              iconD="M3 21h18M6 21V7l6-4 6 4v14M9 9h2M13 9h2M9 13h2M13 13h2"
              tag="Developers"
              ...
              points={['List unlimited projects after approval', 'Commission rates shown upfront to realtors', 'Full lead tracking dashboard', 'Know exactly what you owe — transparent billing', 'Realtor performance analytics per project']}
              cta="List Your Project →" ctaVariant="primary" onCta={dev}
            />
            <ForWhoCard
              iconD="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2M9 7m-4 0a4 4 0 1 0 8 0a4 4 0 1 0-8 0M16 11l2 2 4-4"
              tag="Realtors"
              ...
              points={['Browse projects not listed anywhere else', 'Commission % shown before you submit any lead', 'Real-time lead status tracking', 'Bronze → Silver → Gold → Platinum badge system', 'Commission paid directly to your bank account']}
              cta="Join as Realtor →" ctaVariant="outline" onCta={realtor}
            />
          </div>
        </div>
      </div>
      ...
      <div style={{ background: colors.bg, borderTop: `1px solid ${colors.border}`, padding: '48px 0', overflow: 'hidden' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto 22px', padding: '0 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, color: colors.green, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 7 }}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: colors.green, animation: 'ps-pulse 1.8s ease-in-out infinite' }} />Live on Waseet
            </div>
            <h2 style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.02em', margin: 0 }}>Projects realtors are closing right now</h2>
          </div>
          <span onClick={() => navigate('/marketplace')} style={{ fontSize: 13, color: colors.greenDark, fontWeight: 500, cursor: 'pointer' }}>Browse the marketplace →</span>
        </div>
        <div className="wa-marquee" style={{ position: 'relative' }}>
          <div className="wa-marquee-track" style={{ display: 'flex', gap: 16, width: 'max-content', animation: 'wmarquee 42s linear infinite', padding: '0 8px' }}>
            {[...marqueeCards, ...marqueeCards].map((c, i) => <MarqueeCard key={i} c={c} />)}
          </div>
      ...
      <div style={{ background: '#fff', borderTop: `1px solid ${colors.border}`, padding: '72px 32px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            ...
            <h2 style={{ fontSize: 32, fontWeight: 700, letterSpacing: '-0.02em', margin: 0 }}>Three steps to your first closed deal.</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 40, position: 'relative' }}>
            {[
              { n: '01', d: 'M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6M9 13h6M9 17h4', title: 'Register & get verified', body: 'Apply as a developer or realtor. Our team reviews your documents within 24 hours.' },
              { n: '02', d: 'M11 11m-7 0a7 7 0 1 0 14 0a7 7 0 1 0-14 0M21 21l-4-4', title: 'Browse projects & submit leads', body: 'Realtors browse exclusive projects with commissions shown upfront. Submit client leads instantly.' },
              { n: '03', d: 'M12 12m-9 0a9 9 0 1 0 18 0a9 9 0 1 0-18 0M12 7v10M9.5 9.5c0-1 1.1-1.5 2.5-1.5s2.5.5 2.5 1.5-1.1 1.5-2.5 1.5-2.5.5-2.5 1.5 1.1 1.5 2.5 1.5 2.5-.5 2.5-1.5', title: 'Deal closes — commission paid', body: 'When the deal closes, Waseet tracks payment and disburses your commission to your bank.' },
            ].map((s) => (
              <div key={s.n} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                <div style={{ width: 44, height: 44, borderRadius: '50%', border: `1.5px solid ${colors.border}`, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, marginBottom: 20 }}>{s.n}</div>
                <div style={{ width: 48, height: 48, borderRadius: 12, background: colors.greenTint, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
                  <svg width={24} height={24} viewBox="0 0 24 24" fill="none" stroke={colors.green} strokeWidth={1.7} strokeLinecap="round" strokeLinejoin="round"><path d={s.d} /></svg>
                </div>
          ...
          <div style={{ textAlign: 'center', marginTop: 48 }}>
            <div style={{ fontSize: 14, color: colors.textSoft, marginBottom: 14 }}>Ready to get started?</div>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
              <CtaButton onClick={dev} style={{ height: 40, padding: '0 20px' }}>List Your Project</CtaButton>
              <CtaButton variant="outline" onClick={realtor} style={{ height: 40, padding: '0 20px' }}>Join as Realtor</CtaButton>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: '#0B3D2E', padding: '76px 32px', position: 'relative', overflow: 'hidden', backgroundImage: 'radial-gradient(ellipse at 78% 38%, rgba(34,197,94,0.22) 0%, transparent 55%), repeating-linear-gradient(45deg, rgba(255,255,255,0.02) 0, rgba(255,255,255,0.02) 1px, transparent 1px, transparent 12px)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 52, alignItems: 'center' }}>
          <div>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 999, padding: '6px 14px', fontSize: 13, fontWeight: 600, color: '#fff', marginBottom: 22 }}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#22C55E' }} />Saudi Arabia
            </span>
            <h2 style={{ fontSize: 36, fontWeight: 700, color: '#fff', letterSpacing: '-0.03em', lineHeight: 1.15, margin: '0 0 16px' }}>Private real estate in the world's fastest-growing market.</h2>
            <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.62)', lineHeight: 1.6, margin: '0 0 30px', maxWidth: 440 }}>Vision 2030 is reshaping the Kingdom's skyline. Waseet gives verified realtors first access to the developers building it — with commission locked upfront on every unit.</p>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <CtaButton onClick={() => navigate('/marketplace')} style={{ height: 44, padding: '0 24px' }}>Explore projects</CtaButton>
              <CtaButton variant="dark" onClick={realtor} style={{ height: 44, padding: '0 24px', display: 'flex', alignItems: 'center', gap: 8, color: '#fff' }}>
                <svg width={15} height={15} viewBox="0 0 24 24" fill="#fff"><path d="M8 5v14l11-7z" /></svg>Watch how it works
              </CtaButton>
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              { d: 'M3 21h18M6 21V7l6-4 6 4v14M9 9h2M13 9h2M9 13h2M13 13h2', value: 'SAR 1T+', label: 'in active giga-projects', ml: 0, delay: '0s' },
              { d: 'M3 11l9-8 9 8M5 10v10h14V10M10 20v-6h4v6', value: '1.2M+', label: 'new homes planned by 2030', ml: 28, delay: '.8s' },
              { d: 'M3 17l6-6 4 4 8-8M17 7h4v4', value: '+9.6%', label: 'annual real-estate growth', ml: 0, delay: '1.6s' },
            ].map((s, i) => (
              <div key={i} style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 14, padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 16, marginLeft: s.ml, animation: 'wfloat 4s ease-in-out infinite', animationDelay: s.delay }}>
                <div style={{ width: 46, height: 46, minWidth: 46, borderRadius: 12, background: 'rgba(34,197,94,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width={22} height={22} viewBox="0 0 24 24" fill="none" stroke="#22C55E" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><path d={s.d} /></svg>
                </div>
                <div>
      ...
      <div style={{ background: colors.bg, borderTop: `1px solid ${colors.border}`, padding: '72px 32px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 14 }}>
            {whyCards.map((c, i) => <WhyCard key={i} c={c} />)}
          </div>
        </div>
      </div>
      ...
      <div style={{ background: '#fff', borderTop: `1px solid ${colors.border}`, padding: '56px 32px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-0.02em', margin: '0 0 24px' }}>Browse by property type</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
            {propTypes.map((c, i) => <TypeCard key={i} c={c} navigate={navigate} />)}
          </div>
        </div>
      </div>
      ...
      <div style={{ background: colors.bg, borderTop: `1px solid ${colors.border}`, padding: '72px 32px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', gap: 56, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 300 }}>
            <Eyebrow style={{ marginBottom: 12 }}>Realtor rewards</Eyebrow>
            <h2 style={{ fontSize: 32, fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1.2, margin: '0 0 16px' }}>The more you close,<br />the more you unlock.</h2>
            <p style={{ fontSize: 14, color: colors.textSoft, lineHeight: 1.6, margin: '0 0 28px' }}>Waseet's badge system rewards realtors who consistently deliver. Move from Bronze to Platinum and unlock exclusive benefits with each tier.</p>
            <CtaButton onClick={realtor} style={{ height: 40, padding: '0 20px' }}>Join as Realtor</CtaButton>
          </div>
          <div style={{ flex: 1, minWidth: 300, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {badges.map((b) => (
              <div key={b.name} style={{ background: '#fff', border: `1px solid ${colors.border}`, borderLeft: b.active ? `3px solid ${colors.green}` : `1px solid ${colors.border}`, borderRadius: 10, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
      ...
      <div style={{ background: '#fff', borderTop: `1px solid ${colors.border}`, padding: '72px 32px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontSize: 28, fontWeight: 700, letterSpacing: '-0.02em', textAlign: 'center', margin: '0 0 40px' }}>Trusted by realtors &amp; developers</h2>
            ...
            {reviews.map((r, i) => (
              <div key={i} style={{ background: '#fff', border: `1px solid ${colors.border}`, borderRadius: 12, padding: 22, display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div style={{ display: 'flex', gap: 3 }}>
                  {[...Array(5)].map((_, j) => (
                    <svg key={j} width={16} height={16} viewBox="0 0 24 24" fill="#F59E0B" stroke="#F59E0B" strokeWidth={1}><path d="M12 2l3 6.5 7 .9-5 4.8 1.3 7L12 17.8 5.4 21.2 6.7 14.2 1.7 9.4l7-.9z" /></svg>
                  ))}
                </div>
      ...
      <div style={{ background: colors.bg, borderTop: `1px solid ${colors.border}`, padding: '72px 32px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 44 }}>
            <Eyebrow style={{ fontSize: 11 }}>Built on trust</Eyebrow>
            <h2 style={{ fontSize: 32, fontWeight: 700, letterSpacing: '-0.02em', margin: 0 }}>Verified, protected, and Shariah-compliant.</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16 }}>
            {trustCards.map((c, i) => (
              <div key={i} style={{ background: '#fff', border: `1px solid ${colors.border}`, borderRadius: 14, padding: 26 }}>
                <div style={{ width: 46, height: 46, borderRadius: 12, background: colors.greenTint, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
                  <svg width={23} height={23} viewBox="0 0 24 24" fill="none" stroke={colors.green} strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><path d={c.d} /></svg>
                </div>
      ...
      <div style={{ background: '#fff', borderTop: `1px solid ${colors.border}`, padding: '72px 32px' }}>
        <div style={{ maxWidth: 680, margin: '0 auto' }}>
          <h2 style={{ fontSize: 28, fontWeight: 700, letterSpacing: '-0.02em', textAlign: 'center', margin: '0 0 40px' }}>Frequently asked questions</h2>
      ...
      <div style={{ background: colors.ink, padding: '64px 32px', textAlign: 'center', position: 'relative', overflow: 'hidden', backgroundImage: 'radial-gradient(ellipse at center, rgba(22,163,74,0.08) 0%, transparent 65%)' }}>
        <h2 style={{ fontSize: 40, fontWeight: 700, color: '#fff', letterSpacing: '-0.03em', margin: '0 0 14px' }}>Ready to join Waseet?</h2>
        <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.5)', margin: '0 0 36px' }}>Apply today. Our team reviews within 24 hours.</p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          <CtaButton onClick={dev} style={{ height: 48, padding: '0 32px', fontSize: 15 }}>List Your Project — I'm a Developer</CtaButton>
          <CtaButton variant="dark" onClick={realtor} style={{ height: 48, padding: '0 32px', fontSize: 15, color: 'rgba(255,255,255,0.85)' }}>Join as Realtor — Start Earning</CtaButton>
        </div>
        <div style={{ marginTop: 20, fontSize: 12, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.03em' }}>Free to join · 24hr approval · No commitment</div>
      </div>
    </div>
  )
}
...
/* ---------- ForWho card ---------- */
function ForWhoCard({ iconD, tag, title, body, points, cta, ctaVariant, onCta }) {
  return (
    <div style={{ background: '#fff', border: `1px solid ${colors.border}`, borderRadius: 16, padding: 32 }}>
      ...
      <div style={{ fontSize: 10, fontWeight: 600, color: colors.textFaint, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '20px 0 6px' }}>{tag}</div>
      <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: '-0.02em', marginBottom: 14 }}>{title}</div>
      <div style={{ fontSize: 14, color: colors.textSoft, lineHeight: 1.6, marginBottom: 24 }}>{body}</div>
      ...
      <CtaButton variant={ctaVariant} onClick={onCta} style={{ marginTop: 28, width: '100%', height: 40, fontSize: 14 }}>{cta}</CtaButton>
    </div>
  )
}
...
function HowItWorks() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      if (total > 0) {
        ...
        s = Math.min(2, Math.max(0, Math.floor((scrolled / total) * 3 + 0.001)))
      }
      setActive(s)
    }
  ...
  const goStep = (i) => {
    ...
    const total = w.offsetHeight - window.innerHeight
    window.scrollTo({ top: pageTop + (total * (i + 0.5)) / 3, behavior: 'smooth' })
  }

  const railLabels = ['Browse projects', 'Submit & track leads', 'Get paid commission']
    ...
    <div ref={ref} className="wa-hiw" style={{ position: 'relative', height: '280vh', background: colors.bg, borderTop: `1px solid ${colors.border}` }}>
      <div className="wa-sticky" style={{ position: 'sticky', top: 56, height: 'calc(100vh - 56px)', minHeight: 600, display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
        ...
        <div className="wa-hiw-grid" style={{ position: 'relative', zIndex: 1, maxWidth: 1100, margin: '0 auto', padding: '0 32px', width: '100%', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(330px,1fr))', gap: 56, alignItems: 'center' }}>
          <div>
            <Eyebrow style={{ marginBottom: 24 }}>How Waseet works</Eyebrow>
              ...
              {railLabels.map((label, i) => {
                ...
                return (
                  <div key={i} onClick={() => goStep(i)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', cursor: 'pointer', opacity: on ? 1 : 0.4, transition: 'opacity .2s' }}>
                    <span style={{ width: 30, height: 30, borderRadius: '50%', background: on ? colors.green : colors.surfaceMuted, color: on ? '#fff' : colors.textFaint, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                      {steps[i].n}
                    </span>
  ...
  return (
    <div style={{ flex: 1, padding: 16, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 7, height: 62, marginBottom: 14 }}>
        {bars.map((h, i) => (
          <div key={i} style={{ flex: 1, height: `${h}%`, background: barColor(h), borderRadius: '3px 3px 0 0' }} />
        ))}
      </div>
    ...
    <div style={{ background: colors.ink, padding: '72px 32px', position: 'relative', overflow: 'hidden', backgroundImage: 'radial-gradient(ellipse at 65% 50%, rgba(22,163,74,0.08) 0%, transparent 60%), repeating-linear-gradient(45deg, rgba(255,255,255,0.02) 0, rgba(255,255,255,0.02) 1px, transparent 1px, transparent 12px)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', minHeight: 280 }}>
        <div style={{ maxWidth: 560 }}>
          ...
          <h2 style={{ fontSize: 40, fontWeight: 700, color: '#fff', letterSpacing: '-0.03em', lineHeight: 1.15, margin: '0 0 16px' }}>Exclusive access to Saudi Arabia's finest off-plan projects.</h2>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', lineHeight: 1.6, maxWidth: 460, margin: '0 0 32px' }}>Every verified project shows commission rates upfront — no chasing developers, no surprises.</p>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <CtaButton onClick={() => navigate('/marketplace')} style={{ height: 40, padding: '0 20px', fontSize: 13 }}>Explore projects</CtaButton>
            <CtaButton variant="dark" onClick={() => navigate('/marketplace')} style={{ height: 40, padding: '0 20px', fontSize: 13 }}>View on map</CtaButton>
          </div>
        </div>

        {/* fanned mock cards */}
        <div className="wa-deco" style={{ position: 'absolute', right: 40, top: '50%', transform: 'translateY(-50%)', width: 380, height: 250 }}>
          <div style={{ position: 'absolute', right: 200, top: 30, transform: 'rotate(-4deg)' }}>
            <div style={{ animation: 'wfloat 4.5s ease-in-out infinite', animationDelay: '1.2s', width: 220, background: '#1A1A1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, overflow: 'hidden', boxShadow: '0 14px 40px rgba(0,0,0,0.5)' }}>
              <div style={{ height: 84, background: 'rgba(255,255,255,0.05)', position: 'relative' }}>
                <span style={{ position: 'absolute', top: 9, left: 9, background: colors.greenTint, border: `1px solid ${colors.greenTintBorder}`, borderRadius: 999, padding: '2px 8px', fontSize: 9, fontWeight: 600, color: colors.greenDark }}>NEW</span>
                <span style={{ position: 'absolute', bottom: 8, right: 9, background: colors.green, borderRadius: 5, padding: '2px 8px', fontSize: 10, fontWeight: 600, color: '#fff' }}>4% Commission</span>
              </div>
              <div style={{ padding: '12px 14px' }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: '#fff' }}>Olaya Park Towers</div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', margin: '2px 0 6px' }}>Al Olaya, Riyadh</div>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#fff' }}>SAR 890K – 5.6M</div>
              </div>
            </div>
          </div>
          <div style={{ position: 'absolute', right: 0, top: 8 }}>
            <div style={{ animation: 'wfloat 4.5s ease-in-out infinite', width: 252, background: '#161616', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 14, overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}>
              <div style={{ height: 92, background: 'rgba(255,255,255,0.06)', position: 'relative' }}>
                <span style={{ position: 'absolute', top: 10, left: 10, background: colors.greenTint, border: `1px solid ${colors.greenTintBorder}`, borderRadius: 999, padding: '2px 9px', fontSize: 9, fontWeight: 600, color: colors.greenDark, letterSpacing: '0.03em' }}>FEATURED</span>
                <span style={{ position: 'absolute', bottom: 9, right: 10, background: colors.green, borderRadius: 6, padding: '3px 9px', fontSize: 10, fontWeight: 600, color: '#fff' }}>3% Commission</span>
                <span style={{ position: 'absolute', top: 11, right: 12, fontFamily: 'monospace', fontSize: 9, color: 'rgba(255,255,255,0.2)' }}>project render</span>
              </div>
              <div style={{ padding: '12px 14px' }}>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#fff' }}>Marsa Heights</div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', margin: '2px 0 6px' }}>Al Shati, Jeddah</div>
