...
import { marketplaceGuard } from '../../middleware/maintenance.js'
...
// projects / marketplace — browsing is gated behind marketplace maintenance (admins bypass)
realtorRouter.get('/projects', marketplaceGuard, h((req) => svc.listProjects(req.user.id, req.query).then((projects) => ({ projects }))))
realtorRouter.get('/projects/:id', marketplaceGuard, h((req) => svc.getProject(req.user.id, req.params.id).then((project) => ({ project }))))
realtorRouter.post('/projects/:id/save', marketplaceGuard, h((req) => svc.saveProject(req.user.id, req.params.id)))
...
realtorRouter.get('/marketplace-stats', marketplaceGuard, h((req) => svc.marketplaceStats().then((stats) => ({ stats }))))
...
// disputes
realtorRouter.get('/disputes', h((req) => svc.listDisputes(req.user.id).then((disputes) => ({ disputes }))))
realtorRouter.post('/disputes', h((req) => svc.createDispute(req.user.id, req.body || {}).then((dispute) => ({ dispute }))))
